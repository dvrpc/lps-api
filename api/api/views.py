...
def StationYear(request):
    ...
    for r in rows:  # create json with returned data from super secret db
        record = {"id": r[0], "count": int(r[3])}
        payload.append(record)
    if len(payload) == 0:
        record = {"error": "Internal Server Error", "code": 500}
...
def options(request):
    ...
    cur = conn.cursor()
    cur.execute(
        """SELECT station, surveyyear, operator, mode, line, dvrpc_id FROM points GROUP BY station, surveyyear, operator, mode, line, dvrpc_id ORDER BY station, surveyyear, operator, mode;"""
    )
    rows = cur.fetchall()
    payload = list()
    stations = []
    ...
    for r in rows:
        if str(r[0]) in stations:
            ...
            record = {
                str(r[0]): {"id": r[5], "operator": r[2], "mode": r[3], "line": r[4], "years": []}
            }
